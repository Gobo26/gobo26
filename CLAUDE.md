...
| 2026-08-13 | **`cor-ginesta-cancodeldia.html` renovat amb disseny nou** (capçalera "vinil" + arxiu de targetes, en lloc del disseny anterior de targetes sòbries amb Montserrat): mateix contingut (33 cançons/històries de l'estiu 2026), mateix nom de fitxer i URL (pàgina no llistada a la portada, només enllaç directe pel cor). Corregit `font-family:var(--font-geist-sans)…` (variable de Next.js sense definir en HTML autònom, feia caure la lletra al serif per defecte del navegador) per un stack de sans-serif del sistema |
| 2026-09-21 | **Taller de lletres retirat**: tret l'enllaç/targeta del portal (`index.html`: menú, secció Creació —ara "1 espai"—, footer) i de `README.md`. Repositori `Gobo26/Taller_de_lletres` esborrat de GitHub; còpia local conservada a `~/Taller_de_lletres` |
